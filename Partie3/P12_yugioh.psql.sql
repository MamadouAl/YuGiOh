...
select * from P12_edition where nom_edition ~ '^Bataille';
...
-- 1ERE REQUETE
    ...
    /*
      Après avoir exécuté les requêtes plusieurs fois, on remarque que :
        - la jointure externe prend : 8ms
        - le produit cartésien prend : 17ms
        - la jointure interne prend : 12ms
     */

-- 2EME REQUETE
    -- a.1 Cartes Magie de l'edition 14
    SELECT carte_nom, carte_image
    FROM P12_carte NATURAL JOIN P12_carteEdition
        WHERE num_edition = 14 AND carte_categorie ='Magie';
    ...
    SELECT carte_nom, carte_image
    FROM P12_carte C, P12_carteEdition CE
        WHERE C.num_carte = CE.num_carte AND CE.num_edition = 14 AND C.carte_categorie ='Magie';

    -- d) comparer les temps d’exécution des différentes versions réalisées.
    /*
      Après avoir exécuté les requêtes plusieurs fois, on remarque que :
        - la jointure externe prend : 5ms
        - le produit cartésien prend : 8ms
...
-- 3EME REQUETE
    ...
    /*
      Après avoir exécuté les requêtes plusieurs fois, on remarque que :
        - la jointure externe prend : 12ms
        - le produit cartésien prend : 16ms
        - la jointure interne prend : 6ms
     */
...
--3 Donner une requête pour chacun des opérateurs ensemblistes (UNION, INSERSECT et EXCEPT)
    ...
    SELECT *
    FROM P12_edition WHERE date_edition > '2021-01-01'
    INTERSECT
    SELECT P12_edition.*
...
--4 Donner les requêtes mettant en œuvre les sous-requêtes suivantes :
    ...
    SELECT P12_carte.*  FROM P12_carte NATURAL JOIN P12_carteEdition WHERE carte_rarete='Super Rare'
    ...
    WHERE num_carte IN
          (SELECT num_carte
           FROM P12_carteEdition
           WHERE carte_rarete IN
                            ('Super Rare','Collectors Rare', 'Starlight Rare'));


    --c) une sous-requête dans la clause FROM
    -- LA liste des editions contenant des cartes pièges ayant un type 'CONTINU'
    SELECT P12_edition.*
    FROM (SELECT * FROM P12_carte WHERE carte_categorie = 'Piège' AND carte_type = 'CONTINU') AS CartePiegesContinue
    ...
    SELECT num_carte, carte_nom, carteATK, carte_categorie
    FROM P12_carte natural join P12_carteEdition
        WHERE P12_carteEdition.carte_rarete ='Rare' AND num_edition = ANY (SELECT num_edition FROM P12_edition WHERE date_edition > '2021-01-01');
...
--7 Donner deux requêtes différentes utilisant les fonctions d'agrégation et la clause GROUP BY
    ...
    FROM P12_carte WHERE carteDEF IS NOT NULL
    GROUP BY carte_type;
